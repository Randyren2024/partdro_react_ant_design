...
import { useProductEditor } from '../hooks/useEditor';
...
interface UnifiedProductEditPageProps {}

const UnifiedProductEditPage: React.FC<UnifiedProductEditPageProps> = () => {
  console.log('UnifiedProductEditPage component rendered');
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();
  console.log('UnifiedProductEditPage - id:', id, 'user:', user);
  ...
  // 产品内容编辑器 hook
  const productEditor = isEditMode ? useProductEditor(id!) : null;
  ...
  const handleDelete = async () => {
    ...
    try {
      // 先删除产品相关的内容块
      await EditorService.deleteProductContentBlocks(id);
      // 再删除产品本身
                        ...
                        placeholder='请输入JSON格式的规格参数，例如：
{
  ...
  "最大飞行距离": "8公里"
                  ...
                  <Card>
                    <Space direction="vertical" className="w-full">
                      <Button
                        ...
                        block
                      >
                        {saving ? '保存中...' : (isEditMode ? '更新基础信息' : '创建产品')}
                      </Button>
                      
                      {isEditMode && (
                        <Popconfirm
                          title="确定要删除这个产品吗？"
                          description="此操作不可撤销，将同时删除产品的所有内容"
          ...
          {/* 内容编辑标签页 */}
          <TabPane tab="内容编辑" key="content" disabled={!isEditMode}>
...
export default UnifiedProductEditPage;